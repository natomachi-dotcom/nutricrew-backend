...
function generatePlanEmailHTML(name, lang, plan) {
  ...
  const labelMap = {
    en: { greeting: "Hi", intro: "Your NutriCrew meal plan is ready.", prepLabel: "Prep", tipLabel: "Tip", groceryTitle: "Grocery List", restrictionsTitle: "Food Rules", calories: "kcal", protein: "protein", carbs: "carbs", fat: "fat", printNote: "Print this email or save as PDF to keep your plan offline.", disclaimer: "Generated by AI — for informational purposes only. Consult a healthcare professional before making significant dietary changes." },
    fr: { greeting: "Bonjour", intro: "Votre plan nutritionnel NutriCrew est prêt.", prepLabel: "Préparation", tipLabel: "Conseil", groceryTitle: "Liste de Courses", restrictionsTitle: "Règles Alimentaires", calories: "kcal", protein: "protéines", carbs: "glucides", fat: "lipides", printNote: "Imprimez cet email ou enregistrez-le en PDF pour consulter votre plan hors ligne.", disclaimer: "Généré par IA — à titre informatif uniquement. Consultez un professionnel de santé avant tout changement alimentaire important." },
    es: { greeting: "Hola", intro: "Tu plan nutricional de NutriCrew está listo.", prepLabel: "Preparación", tipLabel: "Consejo", groceryTitle: "Lista de Compras", restrictionsTitle: "Reglas Alimentarias", calories: "kcal", protein: "proteínas", carbs: "carbohidratos", fat: "grasas", printNote: "Imprime este correo o guárdalo como PDF para tener tu plan sin conexión.", disclaimer: "Generado por IA — solo informativo. Consulta a un profesional de salud antes de hacer cambios significativos en tu dieta." },
  };
      ...
      return `
        <tr>
          <td style="padding:16px 24px;border-bottom:1px solid #eee;">
            <div style="font-size:11px;font-weight:bold;text-transform:uppercase;letter-spacing:1px;color:${color};margin-bottom:4px;">${meal.type}</div>
            <div style="font-size:16px;font-weight:bold;color:#0A1628;margin-bottom:4px;">${meal.name}</div>
            <div style="font-size:13px;color:#444;margin-bottom:8px;line-height:1.5;">${meal.description}</div>
            <div style="font-size:12px;color:#555;background:#f5f8ff;border-left:3px solid ${color};padding:8px 10px;margin-bottom:6px;line-height:1.5;"><strong>${L.prepLabel}:</strong> ${meal.prep}</div>
            ${meal.tip ? `<div style="font-size:12px;color:#555;background:#fffbf0;border-left:3px solid #C9A84C;padding:8px 10px;margin-bottom:6px;line-height:1.5;"><strong>${L.tipLabel}:</strong> ${meal.tip}</div>` : ""}
            <div style="font-size:12px;color:#888;margin-top:6px;">${meal.calories} ${L.calories} &nbsp;·&nbsp; ${meal.protein}g ${L.protein} &nbsp;·&nbsp; ${meal.carbs}g ${L.carbs} &nbsp;·&nbsp; ${meal.fat}g ${L.fat}</div>
          </td>
        </tr>`;
    }).join("");

    return `
      <tr><td style="background:#1E3A6E;color:#E8C96A;padding:12px 24px;font-size:15px;font-weight:bold;">${day.label}</td></tr>
      ${day.jetlagNote ? `<tr><td style="background:#152850;color:#7BBFE0;padding:10px 24px;font-size:12px;line-height:1.5;">✈ ${day.jetlagNote}</td></tr>` : ""}
      ${mealsHTML}
      <tr><td style="background:#f0f4ff;color:#0A1628;padding:8px 24px;font-size:12px;text-align:right;">Total: ${day.totalCalories} ${L.calories}</td></tr>`;
  }).join("");
  ...
  const groceryHTML = groceryCats.map(({ key, label }) => {
    ...
    return `
      <td style="vertical-align:top;padding:0 12px 16px 0;width:50%;">
        <div style="font-size:13px;font-weight:bold;color:#4A9ECC;margin-bottom:6px;">${label}</div>
  ...
  const restrictionsHTML = [
    foodRestrictions.usa && `<tr><td style="padding:8px 0;border-bottom:1px solid #eee;"><strong style="color:#4A9ECC;">🇺🇸 USA:</strong><br><span style="font-size:13px;color:#444;line-height:1.6;">${foodRestrictions.usa}</span></td></tr>`,
    foodRestrictions.destination && `<tr><td style="padding:8px 0;border-bottom:1px solid #eee;"><strong style="color:#4A9ECC;">📍 Destination:</strong><br><span style="font-size:13px;color:#444;line-height:1.6;">${foodRestrictions.destination}</span></td></tr>`,
    foodRestrictions.general && `<tr><td style="padding:8px 0;"><strong style="color:#4A9ECC;">💡 General:</strong><br><span style="font-size:13px;color:#444;line-height:1.6;">${foodRestrictions.general}</span></td></tr>`,
  ].filter(Boolean).join("");
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:680px;margin:0 auto;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">

  <!-- Header -->
  <tr><td style="background:#0A1628;padding:28px 24px;text-align:center;">
    <div style="font-size:26px;font-weight:bold;color:#C9A84C;letter-spacing:3px;">✈ NUTRICREW</div>
    <div style="font-size:13px;color:#7BBFE0;margin-top:4px;">Crew Nutrition Plan</div>
  </td></tr>

  <!-- Greeting -->
  <tr><td style="padding:20px 24px;background:#152850;color:#F8FAFF;font-size:14px;line-height:1.6;">
    <strong>${L.greeting}, ${name}!</strong><br>${L.intro}<br>
    <span style="font-size:12px;color:#7BBFE0;">${date} &nbsp;·&nbsp; ${L.printNote}</span>
  </td></tr>

  <!-- Summary -->
  <tr><td style="padding:16px 24px;font-size:14px;color:#333;line-height:1.7;border-bottom:2px solid #C9A84C;">${summary}</td></tr>
  ...
  <tr><td style="padding:20px 24px;">
    <div style="font-size:18px;font-weight:bold;color:#0A1628;border-bottom:2px solid #C9A84C;padding-bottom:8px;margin-bottom:16px;">${L.groceryTitle}</div>
    <table width="100%" cellpadding="0" cellspacing="0"><tr>${groceryHTML}</tr></table>
  </td></tr>

  <!-- Food Rules -->
  <tr><td style="padding:20px 24px;background:#f9f9fb;">
    <div style="font-size:18px;font-weight:bold;color:#0A1628;border-bottom:2px solid #C9A84C;padding-bottom:8px;margin-bottom:12px;">${L.restrictionsTitle}</div>
    <table width="100%" cellpadding="0" cellspacing="0">${restrictionsHTML}</table>
  </td></tr>

  <!-- Footer -->
  <tr><td style="background:#0A1628;padding:16px 24px;text-align:center;">
    <div style="font-size:11px;color:#7A8EAA;line-height:1.6;">${L.disclaimer}</div>
    <div style="font-size:11px;color:#4A6080;margin-top:6px;">© NutriCrew · Fuel Your Flight</div>
  </td></tr>
