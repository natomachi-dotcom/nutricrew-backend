...
function generatePlanEmailHTML(name, lang, destLabel) {
  const labelMap = {
    en: { greeting: "Hi", intro: "is ready.", cta: "Open NutriCrew App", body: "Tap below to view your personalized meals, grocery list, and food rules in the app.", disclaimer: "Generated by AI — for informational purposes only. Consult a healthcare professional before making significant dietary changes." },
    fr: { greeting: "Bonjour", intro: "est prêt.", cta: "Ouvrir l'app NutriCrew", body: "Touchez ci-dessous pour voir vos repas personnalisés, votre liste de courses et vos règles alimentaires dans l'app.", disclaimer: "Généré par IA — à titre informatif uniquement. Consultez un professionnel de santé avant tout changement alimentaire important." },
    es: { greeting: "Hola", intro: "está listo.", cta: "Abrir la app NutriCrew", body: "Toca abajo para ver tus comidas personalizadas, lista de compras y reglas alimentarias en la app.", disclaimer: "Generado por IA — solo informativo. Consulta a un profesional de salud antes de hacer cambios significativos en tu dieta." },
  };
...
async function sendPlanEmail(toEmail, name, lang, destLabel) {
  if (!process.env.RESEND_API_KEY) return;
  const result = await resend.emails.send({
    from: FROM_EMAIL,
    to: [toEmail],
    subject: `✈ Your ${destLabel} meal plan is ready`,
    html: generatePlanEmailHTML(name, lang, destLabel),
  });
